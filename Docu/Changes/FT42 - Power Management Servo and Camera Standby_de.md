...
### Bug #1: Servo Standby funktionierte nicht

**Problem:**
- Servo Standby Button wurde gedrückt, aber Servos blieben steif
- Keine Fehlermeldung, aber keine Wirkung

**Ursache:**
```python
# Move.py Zeile 1897 (FALSCH):
for i in range(16):
    pwm.set_pwm(i, 0)  # ❌ Nur 2 Argumente!
```
- `pwm.set_pwm()` benötigt 3 Argumente: `(channel, on, off)`
- Aufruf mit nur 2 Argumenten verursachte TypeError
- Exception wurde abgefangen, Servos blieben im aktuellen Zustand

**Lösung:**
```python
# Move.py Zeile 1897 (KORREKT):
for i in range(16):
    pwm.set_pwm(i, 0, 0)  # ✅ 3 Argumente!
```

**Effekt:**
- Alle 16 Servo-Kanäle werden korrekt gestoppt
- PWM-Signale auf 0 → Servos werden weich
- Stromverbrauch stark reduziert

---

### Bug #2: Camera Pause funktionierte nicht

**Problem:**
- Camera Pause Button wurde gedrückt
- Video-Stream lief trotzdem weiter
- Keine CPU- oder Bandbreiten-Ersparnis

**Ursache:**
```python
# FPV.py - capture_thread() (FALSCH):
while True:
    frame_image = picam2.capture_array()
    # ... Frame-Verarbeitung ...
    encoded, buffer = cv2.imencode('.jpg', frame_image)
    footage_socket.send(jpg_as_text)  # ❌ Sendet immer!
    time.sleep(0.033)
```
- `camera_paused` Flag wurde in `pause_stream()` gesetzt
- Aber `capture_thread()` prüfte das Flag nie!
- Frames wurden kontinuierlich encodiert und gesendet

**Lösung:**
```python
# FPV.py - capture_thread() (KORREKT):
while True:
    frame_image = picam2.capture_array()
    # ... Frame-Verarbeitung ...
    
    # ✅ Prüfe camera_paused Flag!
...
**Effekt:**
- Frame-Encoding und -Sending übersprungen wenn pausiert
- Kamera läuft weiter (für schnelles Resume)
- Keine Netzwerk-Übertragung → spart Bandbreite
- Kein JPEG-Encoding → spart CPU-Last (~20-30%)

---

### Enhancement: Button-Status-Synchronisation beim GUI-Neustart

**Problem:**
- GUI startet → Buttons zeigen immer "Standby" und "Pause" (blau)
- Aber Server ist bereits in Standby/Pause Modus
- Buttons entsprechen nicht dem tatsächlichen Server-Status
- Benutzer muss zweimal klicken um korrekten Status zu erreichen

**Lösung:**

**1. Server trackt Status (GUIServer.py):**
```python
...
def handle_power_management_command(data):
    if data == 'servo_standby':
        ...
        tcpCliSock.send('servoStandby\n'.encode())  # ← Sendet Status
    
    elif data == 'servo_wakeup':
        move.wakeup()
        servo_standby_active = False
        tcpCliSock.send('servoWakeup\n'.encode())  # ← Sendet Status
    # ... gleiches für camera_pause/resume
```

**2. Server sendet Status beim Connect:**
```python
def info_send_client():
    # Nach VIDEO_READY Phase...
...
**3. Client empfängt und verarbeitet Status (GUI.py):**
```python
def handle_servo_standby_status(active):
    """Update button based on server status"""
...
# ... gleiches für cameraPaused/cameraResumed
```

**Protokoll:**
- Server → Client: `servoStandby\n` / `servoWakeup\n`
- Server → Client: `cameraPaused\n` / `cameraResumed\n`

**Effekt:**
- GUI-Buttons synchronisieren sich automatisch beim Start
- Korrekte Anzeige von Anfang an
- Kein doppeltes Klicken mehr nötig

**Geänderte Dateien (Bugfixes & Enhancement):**
- **Server/Move.py:** `standby()` - Korrektur pwm.set_pwm() Aufruf
- **Server/FPV.py:** `capture_thread()` - camera_paused Flag Prüfung
- **Server/GUIServer.py:** Status-Tracking + Status-Sending bei Connect/Toggle
- **Client/GUI.py:** Handler-Funktionen + connection_thread erweitert
