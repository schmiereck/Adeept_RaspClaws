...
- **Server/FPV.py:** Global pause_stream() und resume_stream() Funktionen + camera_paused Flag-Prüfung
- **Server/GUIServer.py:** handle_power_management_command() Handler + Status-Tracking
- **Client/GUI.py:** 
  - call_servo_standby() und call_camera_pause() Callbacks
  - Btn_ServoStandby und Btn_CameraPause Buttons
  - Keyboard Shortcuts M und ,
  - Status-Synchronisation mit Server

## Updates (2026-01-20)

Nach dem initialen Release wurden einige Bugs behoben und die Status-Synchronisation implementiert:
- Servo Standby funktioniert jetzt korrekt (pwm.set_pwm() Korrektur)
- Camera Pause funktioniert jetzt korrekt (camera_paused Flag wird geprüft)
- Button-Status synchronisiert sich beim GUI-Neustart mit dem Server-Status
- Buttons zeigen immer den korrekten Status an (orange = aktiv, blau = inaktiv)
